...
import { Sparkle, CaretDown, ChartBar, Terminal, Bell, Heartbeat } from '@phosphor-icons/react'
...
function Spark({ data, color = '#0ea5e9', h = 28 }) {
...
function Bars({ data, color = '#0ea5e9', h = 28 }) {
  const max = Math.max(...data)
  const gap = 1
...
const trends = [
  {
    ...
    bg: '#0c2a3b', border: 'rgba(14,165,233,0.3)',
  ...
  },
  {
    ...
    bg: '#0a2a1a', border: 'rgba(34,197,94,0.3)',
    bars: [90, 88, 85, 50, 30, 25, 22, 21, 21, 21],
    barColor: '#22c55e', link: '/explore', linkLabel: 'View trend',
  },
  {
    title: 'SQS queue depth', value: '2.1k',
    sub: 'order-processing queue growing.',
    bg: '#0a2a2a', border: 'rgba(20,184,166,0.3)',
    bars: [20, 25, 30, 38, 50, 65, 80, 95, 100, 105],
    barColor: '#14b8a6', link: '/explore', linkLabel: 'Explore traces',
  },
  {
    ...
    bars: [1, 0, 2, 1, 0, 1, 2],
    barColor: '#94a3b8', link: '/investigate', linkLabel: 'View changes',
  },
]
...
const quickAccess = [
  { icon: Heartbeat, label: 'Health briefing', sub: '12 operational, 2 situations', link: '/monitor', bg: '#0a2a1a', border: 'rgba(34,197,94,0.3)' },
  { icon: ChartBar, label: 'Dashboards', sub: 'payment-service · 2h ago', link: '/explore', bg: '#0c2a3b', border: 'rgba(14,165,233,0.3)' },
  { icon: Terminal, label: 'Logs', sub: 'order-service · 35m ago', link: '/explore', bg: '#1b232d', border: '#424650' },
  { icon: Bell, label: 'Alarms', sub: '4 active', link: '/monitor', bg: '#3b1111', border: 'rgba(239,68,68,0.3)' },
]
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="px-6 py-6 max-w-7xl">
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          {anomalies.map(a => (
            <div key={a.title} onClick={() => navigate(a.link)} className={`rounded-lg p-4 cursor-pointer hover:brightness-110 transition-all ${a.wide ? 'col-span-2' : ''}`} style={{ background: a.bg, border: `1px solid ${a.border}` }}>
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          {trends.map(t => (
            <div key={t.title} onClick={() => navigate(t.link)} className="rounded-lg p-4 cursor-pointer hover:brightness-110 transition-all" style={{ background: t.bg, border: `1px solid ${t.border}` }}>
              ...
              <div className="mt-3"><Bars data={t.bars} color={t.barColor} /></div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {recommendations.map(r => (
            <div key={r.title} onClick={() => openChat(r.chatQuery)} className="rounded-lg p-4 cursor-pointer hover:brightness-110 transition-all" style={{ background: '#06080A', border: '1px solid rgba(224, 64, 251, 0.35)' }}>
        ...
        <div className="grid grid-cols-4 gap-3">
          {quickAccess.map(q => {
            const Icon = q.icon
            return (
              <div key={q.label} onClick={() => navigate(q.link)} className="rounded-lg p-4 cursor-pointer hover:brightness-110 transition-all flex flex-col items-center text-center" style={{ background: q.bg, border: `1px solid ${q.border}` }}>
