          ...
          return (
            <div key={alarm.id} className={`flex items-start gap-2.5 py-2 px-2 rounded-lg hover:bg-primary/5 transition-all text-left group ${isAcked ? 'opacity-50' : ''}`}>
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0 relative opacity-0 group-hover:opacity-100 transition-opacity">
                {!isAcked && <button onClick={(e) => { e.stopPropagation(); setAcked(p => new Set(p).add(alarm.id)) }} className="text-[8px] text-foreground-muted hover:text-status-active h-[20px] px-1.5 rounded bg-background-surface-1 border border-border-muted hover:border-status-active/30 transition-colors inline-flex items-center">Acknowledge</button>}
                <button onClick={(e) => { e.stopPropagation(); setSnoozeMenuOpen(snoozeMenuOpen === alarm.id ? null : alarm.id) }} className="text-[8px] text-foreground-muted hover:text-foreground h-[20px] px-1.5 rounded bg-background-surface-1 border border-border-muted transition-colors inline-flex items-center">Snooze</button>
...
export default function MonitorPage() {
  ...
  const [iacItems, setIacItems] = useState(null)
    ...
    <div className="px-6 py-6 max-w-[1400px] mx-auto">
      ...
      {drawerInvestigation && (
        <>
          ...
          <AgentDrawer investigation={drawerInvestigation} onClose={() => setDrawerInvestigation(null)} onExportCode={(items) => setIacItems(items)} />
        </>
      )}
