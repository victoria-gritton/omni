      ...
      <div className="flex flex-col flex-1 overflow-y-auto -mr-2 pr-2">
        {items.slice(0, 8).map((item) => (
          <div key={item.id} className="flex gap-2 py-2 border-b border-border-muted/20 last:border-0 hover:bg-primary/5 rounded px-1.5 -mx-1.5 cursor-pointer transition-colors" onClick={() => {
            const typeMap = { alarm: 'alarms', coverage: item.title.includes('alarm') ? 'alarms' : item.title.includes('log') ? 'enable-logs' : item.title.includes('trac') ? 'enable-tracing' : 'alarms', insight: 'error-rate', compliance: 'alarms', cost: item.title.includes('log') ? 'optimize-logs' : item.title.includes('stale') ? 'cleanup-stale' : 'alarms' }
