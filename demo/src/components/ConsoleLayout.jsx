...
import { House, Atom, Bell, ChartBar, Globe, TrendUp, Database, Gear, Lifebuoy, MagnifyingGlass, User, X } from '@phosphor-icons/react'
  ...
  return (
    <div ref={ref} className="absolute right-0 top-10 w-[420px] glass-card p-5 z-50 shadow-2xl max-h-[80vh] overflow-y-auto">
      <div className="flex items-start justify-between mb-4">
        ...
        <button onClick={onClose} className="p-1 rounded-lg hover:bg-background-surface-2 text-foreground-muted">
          <X size={14} />
        </button>
      </div>
      ...
      <Section title="Application">
        ...
        <Row label="Accounts" value={application.accounts.map(a => `${a.name} (${a.env})`).join(', ')} />
      </Section>
      ...
      <Section title="Spending">
        ...
        <Row label="CloudWatch spend" value={demo.cloudWatchSpend} />
      </Section>
      ...
      <Section title="Goals for CloudWatch Omni">
        <ul className="flex flex-col gap-1 mt-1">
          {demo.goals.map((g, i) => (
            <li key={i} className="text-[11px] text-foreground-muted flex gap-2">
...
function Section({ title, children }) {
  return (
    <div className="mb-4">
      <h4 className="text-[10px] text-foreground-disabled uppercase tracking-wider font-semibold mb-2">{title}</h4>
...
function Row({ label, value }) {
  return (
    <div className="flex gap-2">
      <span className="text-[11px] text-foreground-muted w-24 flex-shrink-0">{label}</span>
